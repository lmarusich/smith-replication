...
pacman::p_load(knitr, rmarkdown, plyr, tidyverse, afex, ez, BayesFactor, psych,
               ...
               #                , yaml, bookdown, cowplot, pals, viridis, renv, 
               #                parallel, optimParallel, sjstats, 
               #                rstatix, corrplot,
               #                ggstance, trimr)
)
...
observationDataStroop$sj[observationDataStroop$percentLoss>20]
#...None!
...
stroopRT = ddply(trimmedStroopData, 
...
#...get error data
stroopPE = ddply(mergedStroopData, 
                     .(sj, cb,congruency,posture), 
                     summarise, 
...
bfValues = anovaBF(meanRT~congruency*posture+sj,
                   data = stroopBF,
                   whichRandom = "sj", 
                   method="laplace")
bfValues
...
ggsave(interactionPlot, 
       file = "plots/stroop_interaction_plot.pdf", 
       units = "in",
       width = 8.5, 
       height = 5,
       dpi = 600)  
...
write.csv(acc.stats.TS[ ,-7], file = "output/Task_Switching_Descriptives_ACC.csv", 
          row.names = F)


write.csv(accModelTS$anova_table, "output/Task_switching_ANOVA_acc.csv")
accModelTS
...
ggsave(trimmed_RT_plot, 
       file = "plots/TS_trimmed_RT_plot.pdf", 
       units = "in",
       width = 6.62, 
       height = 5.50,
       dpi = 600)
...
write.csv(rtModelTS$anova_table, file = "output/Task_switching_ANOVA_trimmed_RT.csv")
rtModelTS
...
observationDataVS$sj[observationDataVS$percentLoss>20]
#...None!
...
#...get mean error data 
vsPE = ddply(merged.VS.data, 
                 .(sj,cb,setSize, posture), 
                 summarise, 
                 meanPE = 100 - (mean(ac)*100))
head(vsPE)

vsRT = ddply(trimmedData, 
                 .(sj, cb, setSize,posture), 
                 summarise, 
                 meanRT = mean(rt))
...
#...CALCULATE THE BAYES FACTORS FOR THE RT ANALYSIS
bfValues = anovaBF(meanRT~setSize*postureFactor+sj,
                   data = vsCombined,
                   whichRandom = "sj", 
                   method="laplace")
bfValues
...
graphRT = describeBy(vsCombined$meanRT, 
                     list(vsCombined$posture,vsCombined$setSize), 
                     mat=TRUE, 
                     digits = 1)
graphPE = describeBy(vsCombined$meanPE, 
                     list(vsCombined$posture,vsCombined$setSize), 
                     mat=TRUE, 
                     digits = 1)
...
graphRT$se = sqrt((rt.VS.MSE[4])/length(unique(vsCombined$sj)))  
graphPE$se= sqrt((err.VS.MSE[4])/length(unique(vsCombined$sj))) 
...
critT = qt(p=.025,df=length(unique(vsCombined$sj))-2,lower.tail =FALSE)
...
ggsave(interactionPlot, 
       file = "plots/visual_search_interaction_plot.pdf", 
       units = "in",
       width = 8.5, 
       height = 5,
       dpi = 600)  
