...
criteria.effect.sizes <- numeric(length(criteria))
criteria.p.vals <- numeric(length(criteria))
criteria.LLs <- numeric(length(criteria))
criteria.ULs <- numeric(length(criteria))
...
for (i in 1:length(criteria)){
  ...
  tempmod <- aov_ez(data = vsPEcriteria, 
                               ...
                               anova_table = list(es = "pes", correction = "none"),
                               type = 3)
  criteria.effect.sizes[i] <- tempmod$anova_table$pes[3]
  criteria.p.vals[i] <- tempmod$anova_table$`Pr(>F)`[3]
  criteria.LLs[i] <- get.ci.partial.eta.squared( F.value = tempmod$anova_table$F[3],
                              df1 = tempmod$anova_table$`num Df`[3],
                              df2 = tempmod$anova_table$`den Df`[3])$LL
  criteria.ULs[i] <- get.ci.partial.eta.squared( F.value = tempmod$anova_table$F[3],
                              df1 = tempmod$anova_table$`num Df`[3],
                              df2 = tempmod$anova_table$`den Df`[3])$UL
  
}

plot(criteria, criteria.effect.sizes,
     xlab = "Reaction Time Cutoff (ms)",
     ylab = "Interaction Effect Size (PES)",
     pch = 16,
     col = ifelse(criteria.p.vals < 0.05, "red", "black"),
     ylim = c(0,.25))
arrows(x0 = criteria, y0 = criteria.LLs, y1 = criteria.ULs, angle = 90, code = 3)
